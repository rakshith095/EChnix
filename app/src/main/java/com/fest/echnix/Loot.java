...
public class Loot extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		name = (EditText)findViewById(R.id.lootName);
		numb = (EditText)findViewById(R.id.lootNumber);
		sem = (EditText)findViewById(R.id.lootSemester);
		branch = (EditText)findViewById(R.id.lootBranch);
		coll = (EditText)findViewById(R.id.lootColl);
		send = (ImageButton) findViewById(R.id.lootReg);

		TabHost th = (TabHost) findViewById(R.id.tabhostLoot);
		th.setup();
		TabSpec ts = th.newTabSpec("tagLoot1");
		ts.setContent(R.id.lootTab1);
		ts.setIndicator("About");
		th.addTab(ts);
		ts = th.newTabSpec("tagLoot2");
		ts.setContent(R.id.lootTab2);
		...
		send.setOnClickListener(new View.OnClickListener() {			
			...
			public void onClick(View v) {
				...
				if(name.getText().toString().contentEquals("")||
					...
					final ProgressDialog pd = new ProgressDialog(Loot.this);
					pd.setTitle("Registering");
					pd.setMessage("Please Wait...!!");
					pd.show();
					String sent ="SMS_SENT";
			        PendingIntent sentPI = PendingIntent.getBroadcast(Loot.this, 0,new Intent(sent), 0);
